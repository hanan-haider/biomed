...
def test(args, model, test_loader, text_features, seg_mem_features, det_mem_features):
    """Fixed test function with proper batch handling"""
    gt_list = []
    gt_mask_list = []
    ...
    for (image, y, mask) in tqdm(test_loader, desc="Testing"):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            # Process batch tokens correctly
        ...
        for i in range(batch_size):
            # Extract tokens for this specific image
    ...
    print(f"\nEvaluation - Total samples: {len(gt_list)}")
...
def main():
    ...
    augment_fewshot_label = torch.cat([torch.Tensor([1] * len(augment_abnorm_img)), 
                                       torch.Tensor([0] * len(augment_normal_img))], dim=0)
    ...
    for epoch in range(args.epoch):
        ...
        for (image, gt, label) in tqdm(train_loader, desc="Training"):
            image = image.to(device)
