...
_MODEL_CONFIG_PATHS = [Path("/kaggle/input/biomedclip/pytorch/default/1/BiomedCLIP-PubMedBERT-ViT-B-16.json")]
_MODEL_CONFIGS = {}  # directory (model_name: config) of model architecture configs
_MODEL_CKPT_PATHS = {'BiomedCLIP-PubMedBERT-ViT-B-16': Path("/kaggle/input/biomedclip/pytorch/default/1/open_clip_pytorch_model.bin")}
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'microsoft':
        ...
        #print("hereeeeeeeeeeeeee ",_MODEL_CKPT_PATHS[model_name])
        if model_cfg["model_cfg"]["vision_cfg"]["image_size"] != img_size:
            model_cfg["model_cfg"]["vision_cfg"]["image_size"] = img_size
            print("\n Model configuration after changing size ",model_cfg)
            cast_dtype = get_cast_dtype(precision)
            
            
            
            model_pre = load_biomedclip_model(
                            name = _MODEL_CKPT_PATHS[model_name],               # e.g., "microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224"
                            precision=precision,            # e.g., "fp32"
                            device=device,                  # e.g., "cpu" or torch.device("cuda")
                            cache_dir=None,
                            jit=jit# or specify your cache directory
            )
